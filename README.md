# Comm-Log Reconciliation — Xeno Take-Home

Reconciles Finance's reported `target_base` of **22** for merchant 501, October 2026,
across all Diwali campaigns, from the raw `campaign` / `communication_log` data.

**➡️ See [`SUBMISSION.md`](./SUBMISSION.md) for the full reconciliation bridge, the
final SQL query, and a note on what was surprising in the data.**

Exploratory queries used along the way are in [`investigation_queries.sql`](./investigation_queries.sql).
The final, runnable query is in [`solution.sql`](./solution.sql).
...
Or run the final query directly:
...
```
├── README.md                   # this file — data dictionary + pointers
├── SUBMISSION.md                # reconciliation bridge, final SQL, findings (start here)
├── solution.sql                 # final, runnable query -> 22
├── investigation_queries.sql    # exploratory queries used along the way
├── generate_dataset.py          # synthetic dataset generator (unchanged, provided)
└── data/
    ├── comm_log.db
    ├── campaign.csv
